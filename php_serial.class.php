 ...
 * default serial device for osx devices is /dev/tty.serial for
machines with a built in serial device
 *
 ...
 *
 * do not change this code unless you know what you are getting into.
Leaving it as you have received it should work fine!
 */
class phpSerial
{
        var $_device = null;
...
//                      }
                ...
                {
                        trigger_error("Host OS is neither osx, linux nor windows, unable tu
run.", E_USER_ERROR);
                        exit();
                }
        }
...
modes.", E_USER_WARNING);
        ...
        function confBaudRate ($rate)
        {
                ...
                {
                        trigger_error("Unable to set the baud rate : the device is either
...
BAUD=" . $validBauds[$rate], $out);
        ...
        function confParity ($parity)
        {
                if ($this->_dState !== SERIAL_DEVICE_SET)
                {
                        trigger_error("Unable to set parity : the device is either not set
...
either not set or opened", E_USER_WARNING);
        ...
        function confStopBits ($length)
        {
                if ($this->_dState !== SERIAL_DEVICE_SET)
                {
                        trigger_error("Unable to set the length of a stop bit : the device
is either not set or opened", E_USER_WARNING);
                        return false;
                }

                if ($length != 1 and $length != 2 and $length != 1.5 and !($length
== 1.5 and $this->_os === "linux"))
                {
                ...
                if ($this->_os === "linux")
                {
                        $ret = $this->_exec("stty -F " . $this->_device . " " . (($length
== 1) ? "-" : "") . "cstopb", $out);
                }
                else
                {
